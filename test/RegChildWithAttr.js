var RegChildWithAttr = function($,Mustache,Nimbly,PersonData,ListItemComp) {
		...
		"templates":{
			"t4m_template_1":`
				<div>
					...
					<person-data field="testing"></person-data>
					<person-data field="testingb"></person-data>
					<p>START</p>
					<repeat-person-data>
						<person-data>
					</repeat-person-data>
	...
	class componentClass extends Nimbly {
		...
		constructor(data, options) {
			super("RegChildWithAttr", defaults, data || {}, options || {});
		};

		_init() {
			var firstData = $.extend(true, {}, this.data, {"patient_name":"Bobby Bob"});
			var firstPerson = new PersonData(firstData,null);
			this.registerChild(firstPerson, "person-data[field=testing]");
			
			var secondData = $.extend(true, {}, this.data, {"patient_name":"Timmy Tim"});
			var secondPerson = new PersonData(secondData,null);
			this.registerChild(secondPerson, "person-data[field=testingb]");
...
if (typeof module === "undefined") {
	window["RegChildWithAttr"] = RegChildWithAttr($,Mustache,Nimbly,PersonData,ListItemComp);
} else {
	module.exports = function($,Mustache,Nimbly,PersonData,ListItemComp) {
		return RegChildWithAttr($,Mustache,Nimbly,PersonData,ListItemComp);
	};
}
